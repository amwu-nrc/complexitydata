...
atlas_raw_6 <- read_csv(
  "data-raw/atlas_export_data/hs92_country_product_year_6.csv"
) |>
  filter(country_id %in% rankings$country_id, year == 2024)


atlas_economic_complexity92_6 <- calculate_complexity(
  atlas_raw_6,
  region = "country_iso3_code",
  product = "product_hs92_code",
  value = "export_value",
  year = 2024
)

usethis::use_data(atlas_economic_complexity92_6, overwrite = T, compress = "xz")
